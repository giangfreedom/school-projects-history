...
public class Hash {
	...
	public int[] ListofMode;
	
	public Hash(int size){
		HashTable = new int[size];
		CountTable = new int[size];
		ListofMode = new int[size];
		fill();
	}
	...
	public void fill(){
		...
		for(int t = 0; t < ListofMode.length; t++){
			ListofMode[t] = (int) Math.pow(2, 30);
		}
	}
	public void display(){
		...
		for(int t3 = 0; t3 < HashTable.length; t3++){
			//System.out.print(t3 + "         ");
			System.out.print(String.format("%12d",t3));
			//System.out.print(String.format("","%12d", t3));
		}
		System.out.println();
	}
	...
	public int[] modehash(int a[], int n){
		...
		for(int v1 = 0; v1 < CountTable.length; v1++){
			if(CountTable[v1] == max_count){
				ListofMode[index] = HashTable[v1];
				index++;
			}
		}
		index = 0;
